...
if ( ! class_exists( 'WP_PDF_Templates' ) ) 
{
    ...
    class WP_PDF_Templates 
    {
        ...
        public static function define_constants()
        {
            ...
            */
            ...
            if ( ! defined( 'PDF_CACHE_DIRECTORY' ) ) {
                ...
                define('PDF_CACHE_DIRECTORY', $upload_dir['basedir'] . '/pdf/');
        ...
        public static function get_pdf_title( $post_id = null )
        {
            ...
            $pdf_title == get_post_meta( $post_id, '_wp_pdf_templates_title', true );
            #self::pr( $pdf_title );
            ...
            if ( ! $pdf_title ) {
                ...
                // compile pdf title ##
                $pdf_title = $title . '-' . $date;
                
                // tidy up ##
                wp_reset_postdata();
                
                // store pdf title to post_meta ##
                $save = add_post_meta( $post_id, '_wp_pdf_templates_title', $pdf_title, true );
                #self::pr( $save );
                
            }
            
            // kick back a clean title ##
            return apply_filters( 'wp_pdf_templates_title', $pdf_title . '-' . md5($post_id) . '.pdf' );
